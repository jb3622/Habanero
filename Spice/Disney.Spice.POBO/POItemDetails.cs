...
namespace Disney.Spice.POBO
{
    public class POItemDetails : IComparable
    {
        ...
        private short _poasequence;
        public short PoaSequence
        {
            get { return _poasequence; }
            set { _poasequence = value; }
        }
        ...
        public bool ItemLookup(ASNA.VisualRPG.Runtime.Database dbparamref, Disney.Menu.Users username, Disney.Menu.Environments penvironment, string defaultmarket)
        {
            ...
            try
            {
                ...
                if (bSuccess)
                {
                    ...
                    { 
                        app = "Y";
                        itembo.GetAppDescriptions(classcode, vendorcode, stylecode, colorcode, itemsize);
                        itemlongdescription  = itembo.LongDescription;
                        Itemshortdescription = itembo.ShortDescription;
                    }
                ...
                {
                    if (itembo.ItemLongDescription == String.Empty)
                    {
        ...
        public bool ModifyOrderLines(PurchaseOrder _porder, DataTable dtOrderLines, short spicepoversionprevious)
        {
            ...
            if (bSuccessPoItem)
            {
                for (int i = 0; i < _porder.lstpoLineItemDetails.Count; i++)
                {
                    ...
                    if (_porder.lstpoLineItemDetails[i].APP1 == "Y")
                    {
                        ...
                        if (_porder.lstpoLineItemDetails[i]._IsDeleted == false)
                        {
            ...
            if (bSuccessPoItem)
            {
                for (int i = 0; i < _porder.lstpoLineItemDetails.Count; i++)
                {
                    if (_porder.lstpoLineItemDetails[i]._IsDeleted == false)
                    {
                        ...
                        sequence1 = _porder.lstpoLineItemDetails[i].PoaSequence;
        ...
        public int CompareTo(object obj)
        {
            ...
            // ClassCode in not the same so dont bother going any further.
